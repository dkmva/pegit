...
import azimuth3.features.microhomology as microhomology
...
import glob
#import azimuth3
# import azimuth3.models
#import azimuth3.models.ensembles as ensembles
import Bio.Seq as Seq
import time
import scipy.stats as st
import sys
import pandas as pd
import azimuth3.corrstats
...
def qqplot(pvals, fileout = None, alphalevel = 0.05,legend=None,xlim=None,ylim=None,fixaxes=True,addlambda=True,minpval=1e-20,title=None,h1=None,figsize=[5,5],grid=True, markersize=2):
    ...
    for i in xrange(len(pvallist)):        
        ...
        if addlambda:
            ...
            print(f"lambda={lambda_gc:.4f}") % lambda_gc
            ...
            # otherwise add it at the end of the name
                ...
                legendlist[i] = legendlist[i] + f" ($\lambda_{{GC}}=${lambda_gc:.4f})"
...
def target_genes_stats(genes=['HPRT1', 'TADA1', 'NF2', 'TADA2B', 'NF1', 'CUL3', 'MED12', 'CCDC101']):
    for gene in genes:
        ...
        if seq != None:
            print(f'{gene} \t\t\t\t len: {len(seq)} \t GCcont: {SeqUtil.GC(seq):.3f} \t Temp: {Tm.Tm_staluc(seq, rna=False):.4f} \t molweight: {SeqUtil.molecular_weight(seq, "DNA"):.4f}')
            ...
            if isinstance(model_filter, list):
                ...
                for m in model_filter:
                    if m in results_file:
                        in_filt = True
                if not in_filt:
                    print(f"{results_file} not in model_filter") #, model_filter)
                    continue
...
def plot_old_vs_new_feat(results, models, fontsize=20, filename=None, print_output=False):
    ...
    print("old features")
    print(f"mean: {base_spearman_means}")
    print(f"std: {base_spearman_std}")

    print("old + new features")
    print(f"mean: {feat_spearman_means}")
    print(f"std: {feat_spearman_std}")
